...
#if defined(NDEBUG)
#error "Vapexcoin cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zVAPEX spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zVAPEX spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zVAPEX spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
   ...
   if (nHeight == 0) {
      ...
      nSubsidy = 10 * COIN;
    } else if (nHeight >= 110000 && nHeight <= 199999) {
      nSubsidy = 5 * COIN;
   } else if (nHeight >= 200000 && nHeight <= 299999) {
      nSubsidy = 4.5 * COIN;
    } else if (nHeight >= 300000 && nHeight <= 399999) {
      nSubsidy = 4 * COIN;
    } else if (nHeight >= 400000 && nHeight <= 499999) {
      nSubsidy = 3.5 * COIN;
    } else if (nHeight >= 500000 && nHeight <= 599999) {
      nSubsidy = 3 * COIN;
    } else if (nHeight >= 600000 && nHeight <= 699999) {
      nSubsidy = 2.5 * COIN;
    } else if (nHeight >= 700000 && nHeight <= 799999) {
      nSubsidy = 2 * COIN;
    } else if (nHeight >= 800000 && nHeight <= 899999) {
      nSubsidy = 1.5 * COIN;
    } else if (nHeight >= 900000 && nHeight <= 999999) {
      nSubsidy = 1 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Vapexcoin
...
{
    RenameThread("vapexcoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZVAPEXCOINMinted()
{
...
void RecalculateZVAPEXCOINSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_AccumulatorStartHeight()];
    while (true) {
        if (pindex->nHeight % 1000 == 0)
        ...
        //Remove spends from zVAPEX supply
...
bool RecalculateVAPEXCOINSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zVAPEX with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zVAPEXSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert VAPEXCOIN to zVAPEX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zVAPEX spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zVAPEX serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zVAPEX mints and %d zVAPEX spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Vapexcoin: We use certain sporks during IBD, so check to see if they are
