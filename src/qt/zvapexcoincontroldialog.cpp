...
#include "ui_zvapexcoincontroldialog.h"
...
std::list<std::string> ZVapexcoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZVapexcoinControlDialog::listMints;

ZVapexcoinControlDialog::ZVapexcoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZVapexcoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZVapexcoinControlDialog::~ZVapexcoinControlDialog()
{
    delete ui;
}

void ZVapexcoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZVapexcoinControlDialog::updateList()
{
...
void ZVapexcoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZVapexcoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZVapexcoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZVapexcoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZVapexcoinControlDialog::GetSelectedMints()
{
...
void ZVapexcoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
