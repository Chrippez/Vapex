...
#include "zvapexcoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zVAPEX ought to be enough for anybody." - Bill Gates, 2017
    ui->zVAPEXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzVAPEXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzVAPEX->setEnabled(false);
        ui->pushButtonMintzVAPEX->setToolTip(tr("zVAPEX is currently disabled due to maintenance."));

        ui->pushButtonSpendzVAPEX->setEnabled(false);
        ui->pushButtonSpendzVAPEX->setToolTip(tr("zVAPEX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zVAPEXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzVAPEX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVAPEX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zVAPEX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zVAPEX in ") +
...
void PrivacyDialog::on_pushButtonSpendzVAPEX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zVAPEX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zVAPEX
        sendzVAPEX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zVAPEX
    sendzVAPEX();
}

void PrivacyDialog::on_pushButtonZVapexcoinControl_clicked()
{
    ZVapexcoinControlDialog* zVAPEXControl = new ZVapexcoinControlDialog(this);
    zVAPEXControl->setModel(walletModel);
    zVAPEXControl->exec();
}

void PrivacyDialog::setZVapexcoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzVAPEXSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzVAPEX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Vapexcoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zVAPEXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zVAPEX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " VAPEXCOIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zVAPEX</b>";
    ...
    // use mints from zVAPEX selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZVapexcoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZVapexcoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zvapexcoin selector in case it was used
    ZVapexcoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zVAPEX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Vapexcoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zVAPEX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zVAPEX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zVAPEX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zVAPEX "));
    ui->labelzVAPEXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzVAPEXSyncStatus->setVisible(fShow);
}
